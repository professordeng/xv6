...
在 [file.h](https://github.com/professordeng/xv6-expansion/blob/master/file.h) 的一开头就定义了 `file` 结构体，用于一个动态概念的已打开文件，前面刚刚分析过。
